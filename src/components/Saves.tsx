  ...
  return (
    <div className={`w-full h-full flex flex-col ${theme.bgClass} relative overflow-hidden`}>
      <header className={`h-16 shrink-0 border-b flex items-center justify-between px-4 md:px-8 z-20 backdrop-blur-md ${
        isDark ? 'border-white/10 bg-black/40' : 'border-amber-200/50 bg-[#FFFDF9]/60'
      }`}>
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate('/')}
            className={`p-2.5 rounded-xl transition-colors cursor-pointer group ${
              isDark ? 'hover:bg-white/10 text-white' : 'hover:bg-amber-500/10 hover:text-amber-950 text-slate-800'
            }`}
            title="Quay lại"
          >
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className={`w-px h-6 mx-2 hidden md:block ${isDark ? 'bg-white/20' : 'bg-amber-200'}`}></div>
        ...
        <div className="max-w-6xl mx-auto">
          {saves.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-20 opacity-50 space-y-6">
              ...
              <p className={`text-lg lg:text-xl font-bold tracking-widest uppercase text-center ${isDark ? "text-white" : "text-[#5C4033]/70"}`}>Chưa có tệp lưu nào</p>
            </div>
          ) : (
               ...
               <AnimatePresence>
                 {saves.slice().sort((a, b) => b.updatedAt - a.updatedAt).map((save, idx) => (
                   <motion.div
                     ...
                     className={`px-6 py-4 rounded-2xl border transition-all cursor-pointer group relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-4 ${isDark ? 'bg-white/5 border-white/10 hover:border-blue-500/50 hover:bg-white/10' : 'bg-[#FFFDFB] border-amber-200 hover:border-blue-500/60 hover:bg-amber-500/10 shadow-xs'}`}
                   >
                     {/* Background Glow */}
                     <div className="absolute inset-0 bg-gradient-to-r from-blue-500/5 to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity" />
                     
                     <div className="relative z-10 flex-1 min-w-0">
                       <h3 className={`text-lg md:text-xl font-bold break-words ${isDark ? 'text-white' : 'text-[#3E2723]'}`}>
                         {save.name}
                       </h3>
                       <div className={`flex flex-wrap items-center gap-x-6 gap-y-2 mt-2 text-xs md:text-sm opacity-80 ${isDark ? 'text-white/50' : 'text-[#5C4033]/80 font-semibold'}`}>
