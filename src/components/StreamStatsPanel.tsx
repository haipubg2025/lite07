    ...
    <div className="w-full" id="stream-stats-container">
      <div className={`border-y backdrop-blur-md overflow-hidden transition-all ${
        isDark 
          ...
          : 'border-amber-200/40 bg-[#FFFDF9]/60 hover:border-amber-300'
      }`}>
          ...
          {isExpanded && (
            ...
            >
              <div className={`px-4 pb-4 pt-1 border-t font-mono text-[11px] leading-relaxed space-y-2.5 ${
                isDark ? 'border-white/5' : 'border-amber-100'
              }`}>
                
                {/* Trạng thái chính khi chưa có dữ liệu */}
                {!currentStreamStats && !isGeneratingStream ? (
                  <div className={`py-2 text-center italic ${isDark ? 'text-white/30' : 'text-[#3E2723]/40'}`}>
                  ...
                  <>
                    ...
                    <div className="flex flex-col gap-1 py-1">
                      <div className={`flex items-center gap-1.5 font-semibold ${isDark ? 'text-white/40' : 'text-[#5C4033]/80'}`}>
                        <KeyRound className={`w-3 h-3 shrink-0 ${isDark ? 'text-white/30' : 'text-amber-700'}`} />
                        Cổng kết nối:
                      </div>
                      <div className="text-left w-full pl-5">
                        <span className={`font-semibold break-all ${isDark ? 'text-white/80' : 'text-[#3E2723]'}`}>
                          {getConnectionType()}
                        </span>
                    ...
                    <div className="flex flex-col gap-1 py-1 mb-2">
                      <div className={`flex items-center gap-1.5 font-semibold ${isDark ? 'text-white/40' : 'text-[#5C4033]/80'}`}>
                        <Cpu className={`w-3 h-3 shrink-0 ${isDark ? 'text-white/30' : 'text-amber-700'}`} />
                        Model đang chạy:
                      </div>
                    ...
                    <div className="flex justify-between items-center py-0.5">
                      <span className={`flex items-center gap-1.5 ${isDark ? 'text-white/40' : 'text-[#5C4033]/80'}`}>
                        <Zap className={`w-3 h-3 shrink-0 ${isDark ? 'text-white/30' : 'text-amber-600'}`} />
                        Trễ AI:
                      </span>
                      <span className={`text-right font-bold ${isDark ? 'text-yellow-500' : 'text-amber-700'}`}>
                    ...
                    <div className="flex justify-between items-center py-0.5">
                      <span className={`flex items-center gap-1.5 ${isDark ? 'text-white/40' : 'text-[#5C4033]/80'}`}>
                        <Clock className={`w-3 h-3 shrink-0 ${isDark ? 'text-white/30' : 'text-amber-700'}`} />
                        T.gian tổng:
                      </span>
                      <span className={`text-right font-medium ${isDark ? 'text-white/85' : 'text-[#3E2723]'}`}>
                    ...
                    <div className="flex justify-between items-center py-0.5">
                      <span className={`flex items-center gap-1.5 ${isDark ? 'text-white/40' : 'text-[#5C4033]/80'}`}>
                        <FileText className={`w-3 h-3 shrink-0 ${isDark ? 'text-white/30' : 'text-amber-700'}`} />
                        Chữ tạo ra:
                      </span>
                    ...
                    <div className={`pt-2 border-t flex items-center justify-between text-[10px] p-2 rounded-xl ${
                      isDark ? 'border-white/5 bg-black/20 text-white/50' : 'border-amber-200/50 bg-[#F4EFE6] text-[#5C4033] shadow-inner'
                    }`}>
                      <div className="flex flex-col">
                        <span className={`text-[8px] uppercase tracking-widest ${isDark ? 'text-white/30' : 'text-[#5C4033]/60'}`}>Token In</span>
                        <span className={`font-bold ${isDark ? 'text-white/70' : 'text-[#3E2723]'}`}>{currentStreamStats?.inputTokens || '--'}</span>
                      </div>
                      <div className={`h-6 w-px ${isDark ? 'bg-white/5' : 'bg-amber-200/55'}`} />
                      <div className="flex flex-col items-end">
                        <span className={`text-[8px] uppercase tracking-widest ${isDark ? 'text-white/30' : 'text-[#5C4033]/60'}`}>Token Out</span>
                        <span className={`font-bold ${isDark ? 'text-white/70' : 'text-[#3E2723]'}`}>{currentStreamStats?.outputTokens || '--'}</span>
                      </div>
                    </div>
                  </>
                )}
