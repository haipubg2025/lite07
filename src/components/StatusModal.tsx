    ...
    >
      <motion.div 
        ...
        className={`w-full max-w-md rounded-2xl p-6 shadow-2xl relative overflow-hidden border ${
          isDark ? 'bg-[#0e1117]/95 border-slate-700/80' : 'bg-[#FFFDFB] border-amber-200'
        }`}
      ...
      >
        ...
        <button 
          ...
          className={`absolute top-4 right-4 p-2 rounded-xl active:scale-90 transition-all z-10 ${
            isDark 
              ...
              : 'bg-amber-100 text-amber-805 hover:text-[#3E2723] hover:bg-amber-200'
          }`}
          aria-label="Đóng"
        >
          <X size={18} />
        ...
        <div className="flex items-center gap-2 mb-2">
          <span className={`px-2.5 py-0.5 border text-[10px] font-bold uppercase tracking-widest rounded-md ${
            isDark ? 'bg-white/5 border-white/10 text-white/60' : 'bg-amber-100 border-amber-200 text-[#5C4033]'
          }`}>
            {sectionLabel}
          </span>
        </div>

        <h3 className={`text-xl font-bold mb-4 pr-8 line-clamp-2 ${isDark ? 'text-white' : 'text-[#3E2723]'}`}>
          {item.name}
        </h3>

        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className={`p-3 rounded-xl border flex flex-col gap-1 ${
            isDark ? 'bg-white/[0.02] border-white/5' : 'bg-white border-amber-200/65 shadow-xs'
          }`}>
            <span className={`text-[10px] uppercase font-bold tracking-wider ${isDark ? 'text-white/40' : 'text-[#5C4033]/70'}`}>Thời hạn</span>
            {item.type === 'permanent' ? (
          ...
          <div className={`p-3 rounded-xl border flex flex-col gap-1 ${
            ...
            <span className={`text-[10px] uppercase font-bold tracking-wider ${isDark ? 'text-white/40' : 'text-[#5C4033]/70'}`}>Khả năng gỡ</span>
        ...
        <div className={`p-4 rounded-xl border mb-4 max-h-[160px] overflow-y-auto custom-scrollbar ${
          isDark ? 'bg-black/50 border-white/5' : 'bg-amber-500/5 border-amber-200 shadow-inner'
        }`}>
          <span className={`text-[10px] uppercase font-bold block mb-1.5 tracking-wider ${isDark ? 'text-white/40' : 'text-[#5C4033]/70'}`}>Mô tả lý giải</span>
          <p className={`text-xs leading-relaxed whitespace-pre-wrap ${isDark ? 'text-white/90' : 'text-[#3E2723]'}`}>
...
function StatusCard({ 
  ...
  let genderBorderClass = isDark 
    ...
    : 'border-amber-200 hover:border-amber-300 bg-[#FFFDFB] shadow-xs';
  if (isMale) {
      ...
      <div className="flex gap-3 shrink-0 items-start">
        {character.avatar ? (
          ...
          <div className={`w-12 h-16 rounded-xl flex items-center justify-center shrink-0 border ${
            isDark ? 'bg-white/5 border-white/10 text-white/40' : 'bg-amber-100/60 border-amber-200 text-amber-800'
          }`}>
            <User size={22} />
          </div>
        )}
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1.5 flex-wrap">
              <h3 className={`text-sm font-bold break-all whitespace-normal leading-snug ${isDark ? 'text-white' : 'text-[#3E2723]'}`}>
            ...
            <span className={`text-[10px] font-medium ${isDark ? 'text-white/50' : 'text-[#5C4033]/70'}`}>
              ...
              <button 
                ...
                className={`p-2 rounded-xl transition-all border shrink-0 flex items-center justify-center active:scale-95 ${
                  character.isPinned 
                    ...
                    : (isDark ? 'bg-white/5 text-white/50 hover:text-white border-white/5' : 'bg-amber-100/60 hover:bg-amber-100 text-[#5C4033] border-amber-200 hover:text-[#3E2723]')
                }`}
            ...
            <button
              ...
              className={`p-2 rounded-xl transition-all border shrink-0 flex items-center justify-center active:scale-95 ${
                isDark 
                  ...
                  : 'bg-amber-100/60 hover:bg-amber-100 text-[#5C4033] border-amber-200 hover:text-[#3E2723]'
              }`}
            ...
            {isEditing ? (
              <div className="mt-4 pt-4 border-t border-white/10 space-y-4 max-h-[350px] overflow-y-auto custom-scrollbar pr-1">
                {(Object.entries(SECTION_LABELS) as [keyof StatusData, { label: string; icon: any }][]).map(([sectionKey, { label, icon: Icon }]) => {
                  ...
                  return (
                    <div key={sectionKey} className={`rounded-xl border overflow-hidden ${isDark ? 'bg-white/5 border-white/5' : 'bg-amber-100/25 border-amber-200/50'}`}>
                      <div className={`px-2.5 py-1.5 flex items-center gap-2 border-b ${isDark ? 'bg-white/10 border-white/5' : 'bg-amber-100/60 border-amber-200/40'}`}>
                        <Icon size={12} className={isDark ? "text-blue-400" : "text-amber-700"} />
                        <h4 className={`text-[10px] font-bold uppercase tracking-wider ${isDark ? 'text-white' : 'text-[#3E2723]'}`}>{label}</h4>
                      </div>
                      <div className={`p-2 space-y-2`}>
                        {items.length === 0 && (
                          <div className={`text-[10px] italic p-1 ${isDark ? 'text-white/30' : 'text-[#5C4033]/50'}`}>Chưa có trạng thái.</div>
                        )}
                        {items.map((item: StatusItem, i: number) => (
                          <div key={i} className={`relative p-2.5 rounded border space-y-2 ${isDark ? 'bg-black/50 border-white/5' : 'bg-[#FFFDF9] border-amber-200/70'}`}>
                            <input
                              type="text"
                              value={item.name}
                              onChange={(e) => {
                              ...
                              className={`w-full text-[11px] p-1.5 rounded-lg border outline-none focus:border-blue-500/50 ${
                                isDark 
                                  ...
                                  : 'bg-amber-50/50 text-[#3E2723] border-amber-200'
                              }`}
                            ...
                            <textarea
                              ...
                              className={`w-full text-[10px] p-1.5 rounded-lg border outline-none resize-none min-h-[40px] focus:border-blue-500/50 ${
                                isDark 
                                  ...
                                  : 'bg-amber-50/50 text-[#3E2723]/90 border-amber-200'
                              }`}
                              placeholder="Mô tả..."
                            />
                        ...
                        {newItem.section === sectionKey ? (
                          <div className={`p-3 rounded-xl border space-y-2 mt-2 text-xs ${
                            isDark ? 'bg-blue-500/10 border-blue-500/20' : 'bg-amber-500/5 border-amber-250'
                          }`}>
                            <input 
                              type="text" 
                              placeholder="Tên..." 
                              ...
                              <button 
                                ...
                                className={`px-2.5 py-1.5 text-[10px] font-semibold rounded-lg transition-colors ${
                                  isDark 
                                    ...
                                    : 'text-[#5C4033]/70 hover:text-[#3E2723] bg-amber-200/40 hover:bg-amber-200/70'
                                }`}
                              >
                                Hủy
                              </button>
              ...
              <div className={`mt-4 pt-4 border-t ${isDark ? 'border-white/5' : 'border-amber-200/50'}`}>
                {allStatuses.length === 0 ? (
                  <div className={`text-xs italic py-3 text-center border border-dashed rounded-xl ${
                    isDark ? 'text-white/40 bg-white/[0.02] border-white/5' : 'text-[#5C4033]/60 bg-amber-500/5 border-amber-200/50'
                  }`}>
...
export default function StatusModal({ onClose }: StatusModalProps) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className={`fixed inset-0 z-[100] flex flex-col overflow-hidden ${isDark ? 'bg-black/85 backdrop-blur-3xl' : 'bg-[#FAF7F0]/98 backdrop-blur-md font-sans'}`}
    >
      <div className={`flex items-center justify-between px-6 py-4 border-b shrink-0 ${isDark ? 'border-white/10 bg-black/40' : 'border-amber-200/60 bg-[#FFFDF9]/60 backdrop-blur-sm'}`}>
        ...
        <button
          ...
          className={`p-2 rounded-full transition-colors ${
            isDark ? 'bg-white/5 text-white/60 hover:text-white hover:bg-white/10' : 'bg-amber-100 text-[#5C4033] hover:text-[#3E2723] hover:bg-amber-200'
          }`}
        >
          <X size={20} />
        </button>
